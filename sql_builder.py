"""
sql_builder.py — Phase 5: Generate SQLite database and run analytical SQL queries.
Answers the five core business questions from the brief.
"""
import sqlite3
import pandas as pd
import os
...
DB_PATH  = os.path.join(BASE_DIR, "outputs", "customer_intelligence.db")
...
def build_db_and_run(clt_path=CLT_PATH, db_path=DB_PATH):
    print("=" * 60)
    print("PHASE 5 — SQL LAYER")
    print("=" * 60)

    df = pd.read_csv(clt_path)
    print(f"[LOAD] Customer table rows: {len(df):,}")

    # Create SQLite DB
    os.makedirs(os.path.dirname(db_path), exist_ok=True)
    conn = sqlite3.connect(db_path)
    df.to_sql("customers", conn, if_exists="replace", index=False)
    print(f"[DB] Created SQLite DB: {db_path}")
    ...
    conn.close()
    print(f"\n[SQL] All queries executed successfully.")
    print("=" * 60)
    return results

def write_sql_file(sql_path=SQL_PATH):
...
if __name__ == "__main__":
    write_sql_file()
    results = build_db_and_run()
    # Export KPI summary
    kpi_path = os.path.join(BASE_DIR, "outputs", "kpi_summary.csv")
    if "Q7_kpi_summary" in results:
        results["Q7_kpi_summary"].to_csv(kpi_path, index=False)
        print(f"[EXPORT] KPI summary → {kpi_path}")
